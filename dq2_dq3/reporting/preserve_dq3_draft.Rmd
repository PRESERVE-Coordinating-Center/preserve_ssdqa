...
This report summarizes data returned for the second PRESERVE row level query which was distributed to and executed by participating sites in August 2022.
...
attrition_cohort <- results_tbl('attrition_cohort')
...
egfr_bounded <- results_tbl(in_schema('preserve_cro','egfr_bounded_preserve_153'), results_tag=FALSE) %>% 
  inner_join(select(results_tbl("attrition_cohort"), patid), by = "patid")
egfr_plot_df <- make_egfr_dq_df(egfr_tbl = egfr_bounded,
                                cohort = results_tbl("attrition_cohort")) %>% add_site_labels(defer_collect = TRUE)
...
urine_protein_summary_lab_type <- load_codeset("lab_urine_protein_quant") %>%
  ...
  inner_join(results_tbl("attrition_cohort"), by = c("patid", "site")) %>%
...
urine_protein <- load_codeset("lab_urine_protein_quant") %>%
  ...
  inner_join(cdm_tbl("lab_result_cm"),
             by = c("concept_code" = "lab_loinc")) %>%
  inner_join(results_tbl("attrition_cohort"), by = c("patid", "site"))%>% compute_new()
```
