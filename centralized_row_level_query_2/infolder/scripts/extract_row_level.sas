...
/*
output attrition cohort
*/
proc sql;
create table output.attr_cht as
select * from local.attrition_cohort;
quit;

/*
output attrition step 7 cohort
*/
proc sql;
create table output.attr_cht_step7 as
select * from local.ced_step7;
quit;
...
/*diagnoses associated with encounterids to maintain foreign key relationship
*/
proc sql;
create table work.diag_fk as
(select cdm.diagnosisid, cdm.patid, cdm.encounterid, cdm.enc_type, cdm.admit_date,
cdm.providerid, cdm.dx, cdm.dx_type, cdm.dx_date, cdm.dx_source, cdm.dx_origin,
cdm.pdx, cdm.dx_poa, cdm.raw_dx
from indata.diagnosis as cdm
inner join output.encounter as enc
on enc.encounterid = cdm.encounterid);
run;
...
*/
proc sql;
create table work.diag_date_range as
...
select * from work.diag_fk
union (select * from work.diag_date_range);
run;
...
/*
procedures associated with encounterids to maintain foreign key relationship
*/
proc sql;
create table work.proc_fk as
(select cdm.proceduresid, cdm.patid, cdm.encounterid, cdm.enc_type, cdm.admit_date, cdm.providerid, cdm.px_date,
cdm.px, cdm.px_type, cdm.px_source, cdm.ppx, cdm.raw_px, cdm.raw_px_type
...
/*
procedures for cohort within date range
*/
proc sql;
create table work.proc_date_range as
...
select * from work.proc_fk
union (select * from work.proc_date_range);
run;
...
/*
vitals associated with encounterids to maintain foreign key relationship
*/
proc sql;
create table work.vital_fk as
(select cdm.vitalid, cdm.patid, cdm.encounterid, cdm.measure_date, cdm.measure_time, cdm.vital_source, cdm.ht, cdm.wt,
cdm.diastolic, cdm.systolic, cdm.original_bmi, cdm.bp_position, cdm.smoking, cdm.tobacco, cdm.tobacco_type,
cdm.raw_diastolic, cdm.raw_systolic, cdm.raw_bp_position
...
/*
vitals for cohort within date range
*/
proc sql;
create table work.vital_date_range as
...
select * from work.vital_fk
union (select * from work.vital_date_range);
run;
...
/*
lab_result_cm records associated with encounterids to maintain foreign key relationship
*/
proc sql;
create table work.lab_result_cm_fk as
select cdm.lab_result_cm_id, cdm.patid, cdm.encounterid, cdm.specimen_source, cdm.lab_loinc, cdm.lab_result_source,
cdm.lab_loinc_source, cdm.priority, cdm.result_loc, cdm.lab_px, cdm.lab_px_type, cdm.lab_order_date, cdm.specimen_date,
cdm.specimen_time, cdm.result_date, cdm.result_time, cdm.result_qual, cdm.result_snomed, cdm.result_num, cdm.result_modifier,
cdm.result_unit, cdm.norm_range_low, cdm.norm_modifier_low, cdm.norm_range_high, cdm.norm_modifier_high,
cdm.abn_ind, cdm.raw_lab_code, cdm.raw_result, cdm.raw_unit, cdm.raw_lab_name
...
/*
lab_result_cm records for cohort within date range
*/
proc sql;
create table work.lab_result_cm_date_range as
...
abn_ind, raw_lab_code, raw_result, raw_unit, raw_lab_name
from indata.lab_result_cm as cdm
inner join local.broad_ckd_cht as cht
on cht.patid = cdm.patid
...
select * from work.lab_result_cm_fk
union (select * from work.lab_result_cm_date_range);
run;
...
/*
conditions associated with encounterids to maintain foreign key relationship
*/
proc sql;
create table work.cond_fk as
select cdm.conditionid, cdm.patid, cdm.encounterid, cdm.report_date, cdm.resolve_date, cdm.onset_date, cdm.condition_status,
cdm.condition, cdm.condition_type, cdm.condition_source, cdm.raw_condition, cdm.raw_condition_type
...
/*
condition for cohort within date range
*/
proc sql;
create table work.cond_date_range as
...
select * from work.cond_fk
union (select * from work.cond_date_range);
run;
...
/*
prescribing associated with encounterids to maintain foreign key relationship
*/
proc sql;
create table work.pres_fk as
select cdm.prescribingid, cdm.patid, cdm.encounterid, cdm.rx_providerid, cdm.rx_order_date, cdm.rx_order_time,
cdm.rx_start_date, cdm.rx_end_date, cdm.rx_dose_ordered, cdm.rx_dose_ordered_unit, cdm.rx_quantity,
cdm.rx_dose_form, cdm.rx_refills, cdm.rx_days_supply, cdm.rx_frequency, cdm.rx_prn_flag, cdm.rx_route, cdm.rx_basis,
cdm.rxnorm_cui, cdm.rx_source, cdm.rx_dispense_as_written, cdm.raw_rxnorm_cui
...
/*
prescribing for cohort within date range
*/
proc sql;
create table work.pres_date_range as
...
select * from work.pres_fk
union (select * from work.pres_date_range);
run;
...
/*
med_admin records associated with encounterids to maintain foreign key relationship
*/
proc sql;
create table work.med_admin_fk as
select cdm.medadminid, cdm.patid, cdm.encounterid, cdm.prescribingid, cdm.medadmin_providerid, cdm.medadmin_start_date,
cdm.medadmin_start_time, cdm.medadmin_stop_date, cdm.medadmin_stop_time, cdm.medadmin_type, cdm.medadmin_code,
cdm.medadmin_dose_admin, cdm.medadmin_dose_admin_unit, cdm.medadmin_route, cdm.medadmin_source,
cdm.raw_medadmin_code
...
/*
med_admin records for cohort within range
*/
proc sql;
create table work.med_admin_date_range as
...
select * from work.med_admin_fk
union (select * from work.med_admin_date_range);
run;
