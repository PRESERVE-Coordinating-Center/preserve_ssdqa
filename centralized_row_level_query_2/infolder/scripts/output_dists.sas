...
/* Total cohort counts */
proc sql;
create table output.total_cts as
select attr_cht, high_scr, ckd_dx, count(distinct patid) as n_patids from local.comb_cohort
group by attr_cht, high_scr, ckd_dx;
run;
...
select ce_year as ce_calendar_year, attr_cht, high_scr, ckd_dx, count(distinct patid) as n_patids from local.comb_cohort
group by ce_calendar_year, attr_cht, high_scr, ckd_dx;
run;

/* Distribution of CED year of age*/
proc sql;
create table output.dem_ce_age as
select ce_age_years, attr_cht, high_scr, ckd_dx, count(distinct patid) as n_patids from local.comb_cohort
group by ce_age_years, attr_cht, high_scr, ckd_dx;
run;

/* Distribution of birth calendar year*/
proc sql;
create table output.dem_birth_yr as
select birth_calendar_year, attr_cht, high_scr, ckd_dx, count(distinct patid) as n_patids from
(select year(birth_date) as birth_calendar_year, patid, attr_cht, high_scr, ckd_dx from local.cht_demographic)
group by birth_calendar_year, attr_cht, high_scr, ckd_dx;
run;

/* Distribution of demographic categories: Sex, Race, Ethnicity*/
proc sql;
create table output.dem_cats as
select sex, race, hispanic, attr_cht, high_scr, ckd_dx, count(distinct patid) as n_patids from local.cht_demographic
group by sex, race, hispanic, attr_cht, high_scr, ckd_dx;
run;
...
select enc_type, attr_cht, high_scr, ckd_dx, year(admit_date) as year, count (distinct patid) as n_patids, count (distinct encounterid) as n_rows from local.cht_encounter
group by year, enc_type, attr_cht, high_scr, ckd_dx
...
select distinct patid, attr_cht, high_scr, ckd_dx, max(admit_date) as max_enc,
min(admit_date) as min_enc from local.cht_encounter
where enc_type in ('AV', 'ED', 'EI', 'IP', 'OS')
group by patid, attr_cht, high_scr, ckd_dx;
run;
...
/* Distribution of in-person follow-up across ckd_dx cohort */
proc sql;
create table output.enc_ip_fu_ckd_dx as
select  count (distinct patid) as n_patids,
avg(max_enc - min_enc) as mean_days_ip_fu,
std(max_enc - min_enc) as sd_days_ip_fu,
median(max_enc - min_enc) as median_days_ip_fu,
...
/* Distribution of ced to last in-person visit follow-up across ckd_dx cohort */
proc sql;
create table output.enc_ced_fu_ckd_dx as
select count (distinct patid) as n_patids,
avg(max_enc - ce_date) as mean_days_ced_fu,
std(max_enc - ce_date) as sd_days_ced_fu,
median(max_enc - ce_date) as median_days_ced_fu,
...
select enc_type, attr_cht, high_scr, ckd_dx, year(admit_date) as year, count (distinct patid) as n_patids, count (distinct encounterid) as n_rows from local.cht_encounter as enc
...
select vit.patid as patid, dem.attr_cht as attr_cht, dem.high_scr as high_scr, dem.ckd_dx as ckd_dx, floor((measure_date - birth_date)/365.25) as age_years, ht, wt, measure_date, birth_date from
...
select attr_cht, high_scr, ckd_dx, year(measure_date) as year, count (distinct patid) as n_patids, count (distinct vitalid) as n_rows from local.cht_vital
where wt is not null
group by year, attr_cht, high_scr, ckd_dx
order by year;
run;
...
/* Distribution of weight values per patient year of age for ckd_dx */
proc sql;
create table output.vital_wt_vals_ckd_dx as
select distinct count (distinct patid) as n_patids,
age_years, avg(wt) as mean_weight_lb, std(wt) as sd_weight_lb, median(wt) as median_weight_lb,
min(wt) as min_weight_lb,
max(wt) as max_weight_lb from local.vital_ht_wt_age_rows
...
/* Distribution of height values per patient year of age for ckd_dx */
proc sql;
create table output.vital_ht_vals_ckd_dx as
select count (distinct patid) as n_patids,
age_years, avg(ht) as mean_height_in, std(ht) as sd_height_in, median(ht) as median_height_in,
min(wt) as min_height_in,
max(wt) as max_height_in from local.vital_ht_wt_age_rows
...
select dem.attr_cht as attr_cht, dem.high_scr as high_scr, dem.ckd_dx as ckd_dx, vit.patid as patid, floor((measure_date - birth_date)/365.25) as age_years, systolic, diastolic, measure_date, birth_date from
(select * from local.cht_vital
...
/* Distribution of systolic bp values per patient year of age for ckd_dx */
proc sql;
create table output.vital_sys_bp_vals_ckd_dx as
select count (distinct patid) as n_patids,
age_years, avg(systolic) as mean_sys_bp, std(systolic) as sd_sys_bp, median(systolic) as median_sys_bp,
min(systolic) as min_sys_bp,
max(systolic) as max_sys_bp  from local.vital_bp_age_rows
...
/* Distribution of diastolic bp values per patient year of age for ckd_dx */
proc sql;
create table output.vital_dia_bp_vals_ckd_dx as
select count (distinct patid) as n_patids,
age_years, avg(diastolic) as mean_dia_bp, std(diastolic) as sd_dia_bp, median(diastolic) as median_dia_bp,
min(diastolic) as min_dia_bp,
max(diastolic) as max_dia_bp from local.vital_bp_age_rows
...
select attr_cht, high_scr, ckd_dx, dx, dx_type, year(dx_date) as year,
count (distinct patid) as n_patids, count (distinct diagnosisid) as n_rows from local.cht_diagnosis
group by year, dx, dx_type, attr_cht, high_scr, ckd_dx
order by dx, year;
run;
...
select attr_cht, high_scr, ckd_dx, codeset, year(dx_date) as year,
count (distinct patid) as n_patids, count (distinct diagnosisid) as n_rows from local.cht_diagnosis
group by year, codeset, attr_cht, high_scr, ckd_dx
order by year;
run;
...
select attr_cht, high_scr, ckd_dx, condition, condition_type, year(report_date) as year,
count (distinct patid) as n_patids, count (distinct conditionid) as n_rows from local.cht_condition
group by year, condition, condition_type, attr_cht, high_scr, ckd_dx
...
select attr_cht, high_scr, ckd_dx, codeset, year(report_date) as year,
count (distinct patid) as n_patids, count (distinct conditionid) as n_rows from local.cht_condition
group by year, codeset, attr_cht, high_scr, ckd_dx
order by year;
run;
...
select attr_cht, high_scr, ckd_dx, rxnorm_cui, year(coalesce(rx_start_date, rx_order_date)) as year,
count (distinct patid) as n_patids, count (distinct prescribingid) as n_rows from local.cht_prescribing
group by year, rxnorm_cui, attr_cht, high_scr, ckd_dx
...
select attr_cht, high_scr, ckd_dx, codeset, year(coalesce(rx_start_date, rx_order_date)) as year,
count (distinct patid) as n_patids, count (distinct prescribingid) as n_rows from local.cht_prescribing
group by year, codeset, attr_cht, high_scr, ckd_dx
order by year;
run;
...
select attr_cht, high_scr, ckd_dx, ndc, year(dispense_date) as year,
count (distinct patid) as n_patids,
count (distinct dispensingid) as n_rows from local.cht_dispensing
group by year, ndc, attr_cht, high_scr, ckd_dx
order by ndc, year;
run;
...
select attr_cht, high_scr, ckd_dx, codeset, year(dispense_date) as year,
count (distinct patid) as n_patids,
count (distinct dispensingid) as n_rows from local.cht_dispensing
group by year, codeset, attr_cht, high_scr, ckd_dx
...
select attr_cht, high_scr, ckd_dx, year(medadmin_start_date) as year, medadmin_code, medadmin_type,
count (distinct patid) as n_patids, count (distinct medadminid) as n_rows from local.cht_med_admin
group by year, medadmin_code, medadmin_type, attr_cht, high_scr, ckd_dx
...
select attr_cht, high_scr, ckd_dx, year(medadmin_start_date) as year, codeset,
count (distinct patid) as n_patids, count (distinct medadminid) as n_rows from local.cht_med_admin
group by year, codeset, attr_cht, high_scr, ckd_dx
order by year;
run;
...
select attr_cht, high_scr, ckd_dx, px, px_type, year(px_date) as year,
count (distinct patid) as n_patids, count (distinct proceduresid) as n_rows from local.cht_procedures
group by year, px, px_type, attr_cht, high_scr, ckd_dx
order by px, year;
run;
...
select attr_cht, high_scr, ckd_dx, codeset, year(px_date) as year,
count (distinct patid) as n_patids, count (distinct proceduresid) as n_rows from local.cht_procedures
group by year, codeset, attr_cht, high_scr, ckd_dx
order by year;
run;
...
select attr_cht, high_scr, ckd_dx, lab_loinc, year(comb_lab_date) as year,
count (distinct patid) as n_patids, count (distinct lab_result_cm_id) as n_rows from local.cht_lab_result_cm
group by year, lab_loinc, attr_cht, high_scr, ckd_dx
...
select distinct attr_cht, high_scr, ckd_dx, codeset, year(comb_lab_date) as year,
count (distinct patid) as n_patids, count (distinct lab_result_cm_id) as n_rows from local.cht_lab_result_cm
group by year, lab_loinc, attr_cht, high_scr, ckd_dx
order by codeset, year;
run;
...
select attr_cht, high_scr, ckd_dx, lab_loinc, result_qual, result_snomed, count (distinct patid) as n_patids,
count (distinct lab_result_cm_id) as n_rows from local.cht_lab_result_cm
group by lab_loinc, result_qual, result_snomed, attr_cht, high_scr, ckd_dx
...
/*
result_num summary statistics for each result_unit and key lab_loinc code combination for ckd_dx
*/
proc sql;
create table output.lab_vals2_ckd_dx as
select count (distinct patid) as n_patids,
lab_loinc, result_unit, avg(result_num) as mean_result_num,
median(result_num) as median_result_num,
std(result_num) as sd_result_num,
...
select attr_cht, high_scr, ckd_dx, lab_loinc, result_unit, norm_range_high, norm_range_low,
count (distinct patid) as n_patids, count (distinct lab_result_cm_id) as n_rows from local.cht_lab_result_cm
group by lab_loinc, result_unit, norm_range_high, norm_range_low, attr_cht, high_scr, ckd_dx;
run;
...
select * from
(select attr_cht, high_scr, ckd_dx, lab_loinc, concept_name, raw_result, result_qual,
count (distinct patid) as n_patids, count (distinct lab_result_cm_id) as n_rows from
((select attr_cht, high_scr, ckd_dx, patid, lab_loinc, raw_result, result_qual, lab_result_cm_id from local.cht_lab_result_cm
...
group by lab_loinc, concept_name, result_qual, raw_result, attr_cht, high_scr, ckd_dx)
where n_patids >= 3;
run;
...
select * from
(select attr_cht, high_scr, ckd_dx, lab_loinc, raw_lab_name,
count (distinct patid) as n_patids, count (distinct lab_result_cm_id) as n_rows from local.cht_lab_result_cm
group by lab_loinc, raw_lab_name, attr_cht, high_scr, ckd_dx)
where n_patids >= 3;
run;
