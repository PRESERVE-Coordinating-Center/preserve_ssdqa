...
select concept_code, pcornet_vocabulary_id, 'dx_ckd_allstages' as codeset from infolder.dx_ckd_allstages
union
...
select cdm.patid, cdm.birth_date, sex, hispanic, race, ce_date, ce_year, attr_cht, ckd_dx, high_scr, ce_age_years from output.demographic as cdm
...
select cht.patid, attr_cht, high_scr, ckd_dx, encounterid, ce_date, admit_date, discharge_date, enc_type, facility_type, raw_facility_type, providerid,
payer_type_primary, payer_type_secondary
from output.encounter as cdm
inner join local.comb_cohort as cht
on cdm.patid = cht.patid;
run;
...
select cht.patid, attr_cht, high_scr, ckd_dx, vitalid, encounterid, measure_date, ht, wt, diastolic, systolic, original_bmi
from output.vital as cdm
inner join local.comb_cohort as cht
on cdm.patid = cht.patid;
run;
...
select cht.patid, attr_cht, high_scr, ckd_dx, codeset, diagnosisid, encounterid, dx, dx_type, dx_date, raw_dx, providerid
from output.diagnosis as cdm
inner join local.comb_cohort as cht
on cdm.patid = cht.patid
...
select cht.patid, attr_cht, high_scr, ckd_dx, codeset, conditionid, encounterid, report_date, resolve_date, onset_date, condition, condition_type, raw_condition
from output.condition as cdm
inner join local.comb_cohort as cht
on cdm.patid = cht.patid
inner join local.all_dx as cds
...
select cht.patid, attr_cht, high_scr, ckd_dx, codeset, prescribingid, encounterid, rx_order_date, rx_start_date, rx_end_date, rxnorm_cui,
rx_route, rx_providerid
from output.prescribing as cdm
inner join local.comb_cohort as cht
on cdm.patid = cht.patid
inner join
...
select cht.patid, attr_cht, high_scr, ckd_dx, codeset, dispensingid, dispense_date, ndc,
dispense_route
from output.dispensing as cdm
inner join local.comb_cohort as cht
on cdm.patid = cht.patid
inner join
(select concept_code, pcornet_vocabulary_id, codeset from local.all_rx
...
select cht.patid, attr_cht, high_scr, ckd_dx, medadminid, encounterid, medadmin_start_date, medadmin_stop_date, medadmin_code, medadmin_type,
medadmin_route, medadmin_providerid
from output.med_admin as cdm
inner join local.comb_cohort as cht
on cdm.patid = cht.patid;
run;
proc sql;
...
select cht.patid, attr_cht, high_scr, ckd_dx, proceduresid, admit_date, px_date, px, px_type, raw_px, providerid
from output.procedures as cdm
inner join local.comb_cohort as cht
on cdm.patid = cht.patid;
run;
proc sql;
...
select cht.patid, attr_cht, high_scr, ckd_dx, codeset, lab_result_cm_id, lab_loinc,
