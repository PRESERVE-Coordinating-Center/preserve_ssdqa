...
.run  <- function(base_dir = config('base_dir')) {
  ...
  dirs <- paste0(base_dir, '/results/preserve_site_output/')
  dirs_dc <- paste0(base_dir, '/results/preserve_dc_output/')
  dirs_rl <- paste0(base_dir, '/results/preserve_rl_agg_output/')
  dirs_rl_2 <- paste0(base_dir, '/results/preserve_rl2_agg_output/')
  substring_names <- c("codeset_cts","enc_ced_fu",
                    "enc_ip_fu","lab_vals2","vital_dia_bp_vals",
                    "vital_ht_vals","vital_sys_bp_vals","vital_wt_vals")
  upload_sites(dirs, substr_names=substring_names)
  upload_sites(dirs_dc, substr_names=substring_names)
  upload_combined(dirs, res_schema_name='pcc_feas', substr_names=substring_names)
  upload_combined(dirs_dc, res_schema_name='pcc_dc')
  upload_sites(dirs_rl, substr_names=substring_names)
  upload_combined(dirs_rl, res_schema_name='preserve_rl1_agg', substr_names=substring_names)
  upload_sites(dirs_rl_2, substr_names=substring_names)
  upload_combined(dirs_rl_2, res_schema_name='preserve_rl2_agg', substr_names=substring_names)

  # Output site mask mapping to db
  read_csv("specs/site_report_config.csv", col_types = "cccc") %>%
    select(site_name, site_mask) %>%
    output_tbl("_site_name_mask_maps")
